...
class MeshDyn():
    ...
    meshsize=property(getMeshsize,setMeshsize)    
        ...
        def plotRoomPart(self):
            ...
            X, Y = np.meshgrid(X, Y)
            numberOfXNodes = self.xLength/self.meshsize+1
            numberOfYNodes = self.yLength/self.meshsize+1
            Z = np.array([[self.nodeMatrix[i,j].funcVal for i in range(numberOfYNodes)] \
