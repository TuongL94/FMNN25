...
class Mesh():
    ...
    def __updateMeshValues__(self, roomNbr, newSolution):
        ...
        for i in range(0, self.y_res - 2):
            for j in range(0, self.x_res - 2):
                self.grid[1 + i][1 + j].setFuncAndPrevFuncVal(newSolution.item(index))
                index += 1
        # Computes the temperatures at the right boundary and stores them in the corresponding mesh nodes
        if roomNbr == 1:
            for i in range(0,self.y_res - 2):
                self.grid[1+i][self.x_res - 1].setFuncAndPrevFuncVal(newSolution.item(self.x_res - 3 + i * min(self.x_res -2, self.y_res - 2)) + self._stepSize * self.grid[1+i][self.x_res - 1].getDeriv())
